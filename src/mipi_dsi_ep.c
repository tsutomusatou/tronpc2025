...
void mipi_dsi_start_display(void)
{
    ...
    err = R_ICU_ExternalIrqEnable(&g_int_gt911_ctrl);
    /* Handle error */
...
{
    if (g_int_gt911_cfg.channel == p_args->channel)
    {
        g_irq_state =true;
    }
}
...
void handle_error (fsp_err_t err,  const char * err_str)
{
    if(FSP_SUCCESS != err)
    {
        ...
        if(RESET_VALUE != g_int_gt911_ctrl.open)
        {
            if(FSP_SUCCESS != R_ICU_ExternalIrqClose (&g_int_gt911_ctrl))
            {
        ...
        if(RESET_VALUE != g_i2c_gt911_ctrl.open)
        {
            if(FSP_SUCCESS != R_IIC_MASTER_Close(&g_i2c_gt911_ctrl))
            {
...
void mipi_dsi_entry(void)
{
    ...
    err = R_IIC_MASTER_Open(&g_i2c_gt911_ctrl, &g_i2c_gt911_cfg);
    /* Handle error */
    ...
    err = R_ICU_ExternalIrqOpen(&g_int_gt911_ctrl, &g_int_gt911_cfg);
    /* Handle error */
